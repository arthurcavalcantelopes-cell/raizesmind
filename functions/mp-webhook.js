...
// V19 mudancas: aceita external_reference como UUID (userId direto) OU email
// V18: UPSERT defensivo, billing_history idempotente, Resend recibo
...
export async function onRequest(context){
  ...
  try {
    ...
    // V19: external_reference pode ser UUID ou email — extrair os dois
    ...
    // V19: lookup user_id robusto — tenta UUID direto, depois email
    let userId = null;
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(externalRef);

    if(isUuid){
      // external_reference É o userId — confirma que existe no profiles
      try {
        ...
        if(profs && profs[0]){ userId = profs[0].id; console.log('[mp-webhook] user found by userId UUID:', userId); }
      } catch(e){ console.error('[mp-webhook] UUID lookup error:', e); }
    }

    // Fallback: tenta achar por email (do metadata, depois payer, depois externalRef)
    if(!userId){
    ...
    // E-mail pra notificacao: prioriza meta, depois payer, depois externalRef
    const notifyEmail = metaEmail || payerEmail || (externalRef.includes('@') ? externalRef : null);

    // 2. Sempre grava em billing_history (idempotente via payment_id)
    try {
    ...
    if(status === 'approved'){
      ...
      // 4. Email recibo via Resend (so se temos email)
      if(RESEND_KEY && notifyEmail){
        ...
        console.warn('[mp-webhook] sem email para enviar recibo');
      } else {
        console.warn('[mp-webhook] RESEND_API_KEY missing, recibo não enviado');
      }
    }
