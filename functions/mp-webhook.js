// functions/mp-webhook.js - V19 BUGFIX
...
export async function onRequest(context) {
  ...
  if (topic !== "payment" || !id) {
    console.log("[mp-webhook] ignored - not payment notification:", topic, id);
  ...
  try {
    ...
    if (!r.ok) {
      console.error("[mp-webhook] MP fetch failed:", r.status);
      return new Response("Fetch failed but ack", { status: 200 });
    }
    ...
    const payerEmail = String((payment.payer && payment.payer.email) || "").toLowerCase().trim();
    const amount = payment.transaction_amount;
    const method = meta.method || payment.payment_method_id || "cartao";
    const cardLast4 = (payment.card && payment.card.last_four_digits) || null;
    const cardBrand = (payment.payment_method && payment.payment_method.id) || null;
    ...
    if (isUuid) {
      try {
        ...
        if (profs && profs[0]) {
          userId = profs[0].id;
          console.log("[mp-webhook] user found by UUID:", userId);
        }
    ...
    if (!userId) {
      ...
      for (const candidate of candidates) {
        try {
          ...
          if (profs && profs[0]) {
            userId = profs[0].id;
            console.log("[mp-webhook] user found by email:", candidate, "to", userId);
            break;
          }
    ...
    if (status === "approved") {
      ...
      if (RESEND_KEY && notifyEmail) {
        try {
          ...
          const methodLabel = method === "cartao" || method === "credit_card" ? "Cartao de credito" : method === "pix" ? "PIX" : method === "boleto" ? "Boleto" : method;
          ...
          const nextBillFmt = new Date(nextBilling).toLocaleDateString("pt-BR");
          const html = '<!DOCTYPE html><html lang="pt-BR"><body style="margin:0;padding:0;background:#f8f3ea;font-family:Arial,sans-serif;color:#1e150b;"><div style="max-width:560px;margin:0 auto;background:#fff;border-radius:.7rem;overflow:hidden;"><div style="background:linear-gradient(135deg,#3d5230,#5a7a3e);color:#fff;padding:2rem 1.6rem;text-align:center;"><div style="font-size:2.5rem;">OK</div><div style="font-family:Georgia,serif;font-size:1.5rem;margin-top:.4rem;">Pagamento aprovado</div><div style="font-size:.84rem;opacity:.9;margin-top:.2rem;">Sua assinatura Raizes esta ativa</div></div><div style="padding:2rem 1.8rem;font-size:.95rem;line-height:1.7;"><p style="text-align:center;color:#5a3a1e;margin-bottom:1.5rem;">Obrigado por usar a Raizes</p><div style="background:#f6f1e8;border-radius:.55rem;padding:1.2rem 1.4rem;font-size:.88rem;"><div><strong>Valor pago:</strong> R$ ' + fmt(amount) + '</div><div><strong>Metodo:</strong> ' + methodLabel + '</div><div><strong>Ciclo:</strong> ' + cycleLabel + '</div><div><strong>Proximo vencimento:</strong> ' + nextBillFmt + '</div><div><strong>Data:</strong> ' + dataFmt + '</div><div><strong>ID transacao:</strong> ' + id + '</div></div><p style="font-size:.78rem;color:#7a5c40;margin-top:1.5rem;background:#fef9f3;padding:.8rem 1rem;border-radius:.4rem;">Recibo provisorio (fase 1 PF). NFSe via SLU em breve. Suporte: <a href="mailto:financeiro@raizesmind.com.br">financeiro@raizesmind.com.br</a></p><p style="font-size:.78rem;color:#7a5c40;text-align:center;font-style:italic;margin-top:1.5rem;"><strong>Equipe Raizes</strong><br><a href="' + SITE_URL + '/">raizesmind.com.br</a></p></div></div></body></html>';
          await fetch("https://api.resend.com/emails", {
            method: "POST",
            headers: { "Authorization": "Bearer " + RESEND_KEY, "Content-Type": "application/json" },
            body: JSON.stringify({
              from: "Raizes <noreply@raizesmind.com.br>",
              to: [notifyEmail],
              reply_to: "financeiro@raizesmind.com.br",
              subject: "Pagamento aprovado - Recibo Raizes",
              html: html
            })
          });
          console.log("[mp-webhook] recibo enviado para:", notifyEmail);
        } catch (e) { console.error("[mp-webhook] resend error:", e); }
      }
    }

    return new Response("OK status=" + status, { status: 200 });
  } catch (err) {
    console.error("[mp-webhook] FATAL:", err);
