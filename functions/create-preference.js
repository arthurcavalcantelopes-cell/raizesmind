...
export async function onRequest(context){
  ...
  let body;
  try { body = await request.json(); } catch(e){ return new Response(JSON.stringify({ error: 'Invalid JSON' }), { status: 400, headers: cors() }); }
  const { plan, cycle, method, email, name, userId } = body;
  if(!email || !name || !plan || !cycle || !method) return new Response(JSON.stringify({ error: 'Campos obrigatorios' }), { status: 400, headers: cors() });
  ...
  } catch(err){ return new Response(JSON.stringify({ error: 'Falha de rede', details: String(err) }), { status: 500, headers: cors() }); }
}
